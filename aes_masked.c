#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <openssl/rand.h>
#include "aes_masked.h"
#include <time.h>


#define NOP_1    ({__asm__ __volatile__("nop"::);})
#define NOP_10   ({ NOP_1; NOP_1; NOP_1; NOP_1; NOP_1; NOP_1; NOP_1; NOP_1; NOP_1; NOP_1;})
#define NOP_100  ({ NOP_10; NOP_10; NOP_10; NOP_10; NOP_10; NOP_10; NOP_10; NOP_10; NOP_10; NOP_10;})
#define NOP_1000 ({ NOP_100; NOP_100; NOP_100; NOP_100; NOP_100; NOP_100; NOP_100; NOP_100; NOP_100; NOP_100;})


uint8_t Sbox_masked[256];
uint8_t sbox[256] = {
    
};

const uint8_t rcon[10] = { 1, 2, 4, 8, 16, 32, 64, 128, 27, 54 };


/**
 * @brief Performs a cyclic row shift in a 4x4 state matrix.
 * @param x The 4x4 state matrix to transform.
 */
void shift_rows(uint8_t x[4][4]) {
    uint8_t tmp = x[1][0];
    x[1][0] = x[1][1];
    x[1][1] = x[1][2];
    x[1][2] = x[1][3];
    x[1][3] = tmp;

    tmp = x[2][0];
    x[2][0] = x[2][2];
    x[2][2] = tmp;;
    tmp = x[2][1];
    x[2][1] = x[2][3];
    x[2][3] = tmp;

    tmp = x[3][0];
    x[3][0] = x[3][3];
    x[3][3] = x[3][2];
    x[3][2] = x[3][1];
    x[3][1] = tmp;
}


/**
 * @brief Mixes a column in the state matrix using Rijndael's Galois field.
 * @param r The column array of size 4 to be mixed.
 */
void mix_column(uint8_t r[4]) {
    uint8_t a[4];
    uint8_t b[4];
    /* The array 'a' is simply a copy of the input array 'r'
     * The array 'b' is each element of the array 'a' multiplied by 2
     * in Rijndael's Galois field
     * a[n] ^ b[n] is element n multiplied by 3 in Rijndael's Galois field */ 
    for (int8_t c = 0; c < 4; c++) {
        a[c] = r[c];
        /* h is 0xff if the high bit of r[c] is set, 0 otherwise */
        uint8_t h = (uint8_t) ((signed char) r[c] >> 7); /* arithmetic right shift, thus shifting in either zeros or ones */
        b[c] = r[c] << 1; /* implicitly removes high bit because b[c] is an 8-bit char, so we xor by 0x1b and not 0x11b in the next line */
        b[c] ^= 0x1B & h; /* Rijndael's Galois field */
    }
    r[0] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1]; /* 2 * a0 + a3 + a2 + 3 * a1 */
    r[1] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2]; /* 2 * a1 + a0 + a3 + 3 * a2 */
    r[2] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3]; /* 2 * a2 + a1 + a0 + 3 * a3 */
    r[3] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0]; /* 2 * a3 + a2 + a1 + 3 * a0 */
}


/**
 * @brief Applies the round key to the state matrix for a specific round.
 * @param x The 4x4 state matrix.
 * @param round The current round number.
 * @param round_key The array of precomputed round keys.
 */
void add_round_key(uint8_t x[4][4], int32_t round, uint8_t round_key[176]) {
    for (int32_t i = 0; i < 4; i++) { 
        for (int32_t j = 0; j < 4; j++) { 
            x[j][i] ^= round_key[(round << 4) + (i << 2) + j];
        }
    }
}


/**
 * @brief Initializes the masked S-box based on the provided masks.
 * @param m The main mask.
 * @param m_prime The secondary mask.
 */
void init_masked_sbox(const uint8_t m, const uint8_t m_prime) {
    for (int32_t i = 0; i < 256; i++) {
        Sbox_masked[i ^ m] = sbox[i] ^ m_prime;
    }
}

/**
 * @brief Applies the SubBytes transformation using the masked S-box.
 * @param x The 4x4 state matrix to transform.
 */
void sub_byte(uint8_t x[4][4]) {
    for (int32_t i = 0; i < 4; i += 1) {
        for (int32_t j = 0; j < 4; j += 1) {
            x[i][j] = Sbox_masked[x[i][j]];
        }
    }
}


/**
 * @brief Changes the masking of the state matrix using new masks.
 * @param x The 4x4 state matrix.
 * @param mp The new mask M'.
 * @param mt The array of transformations for the mask M.
 */
void change_masks(uint8_t x[4][4], const uint8_t mp, const uint8_t* mt) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            x[i][j] = (x[i][j] ^ mt[i]) ^ mp;
        }
    }
}

/**
 * @brief Initializes the state matrix with plaintext data.
 * @param x The 4x4 state matrix to initialize.
 * @param plain_text The plaintext input array (16 bytes).
 */
void init(uint8_t x[4][4], uint8_t plain_text[16]) {
    for (int32_t i = 0; i < 4; i += 1) {
        for (int32_t j = 0; j < 4; j += 1) {
            x[j][i] = plain_text[i * 4 + j];
        }
    }
}

/**
 * @brief Applies masking to the plaintext before encryption.
 * @param x The 4x4 state matrix containing the plaintext.
 * @param mpt The mask array to apply to the plaintext.
 */
void mask_plain_text(uint8_t x[4][4], uint8_t mpt[4]) {
    for (int i = 0; i < 4; i++) {
        x[i][0] ^= mpt[i];
        x[i][1] ^= mpt[i];
        x[i][2] ^= mpt[i];
        x[i][3] ^= mpt[i];
    }
}

/**
 * @brief Generates the masked round keys for the AES encryption process.
 * @param key The original key.
 * @param round_key The output array for the precomputed round keys.
 * @param m The main mask.
 * @param mp The secondary mask M'.
 * @param mpt The mask transformation array for the initial keys.
 */
void key_schedule_masked(const uint8_t *key, uint8_t *round_key, uint8_t m, const uint8_t mp, const uint8_t *mpt) {
    
    for (int i = 0; i < 4; i++) {
        round_key[i * 4]= key[i * 4]     ^ mpt[0] ^ m;
        round_key[i * 4 + 1] = key[i * 4 + 1] ^ mpt[1] ^ m;
        round_key[i * 4 + 2] = key[i * 4 + 2] ^ mpt[2] ^ m;
        round_key[i * 4 + 3] = key[i * 4 + 3] ^ mpt[3] ^ m;
    }

    for (int i = 4; i < 40; i++) {
        uint8_t temp[4];
        for (int j = 0; j < 4; j++) {
            temp[j] = round_key[(i - 1) * 4 + j];
        }

        if (i % 4 == 0) {
            uint8_t k = temp[0] ^ mpt[0];
            temp[0] = temp[1] ^ mpt[1];
            temp[1] = temp[2] ^ mpt[2];
            temp[2] = temp[3] ^ mpt[3];
            temp[3] = k;

            temp[0] = Sbox_masked[temp[0]];
            temp[1] = Sbox_masked[temp[1]];
            temp[2] = Sbox_masked[temp[2]];
            temp[3] = Sbox_masked[temp[3]];

            temp[0] ^= rcon[i / 4 - 1];

            round_key[i * 4 + 0] = (round_key[(i - 4) * 4 + 0] ^ temp[0]) ^ mp;
            round_key[i * 4 + 1] = (round_key[(i - 4) * 4 + 1] ^ temp[1]) ^ mp;
            round_key[i * 4 + 2] = (round_key[(i - 4) * 4 + 2] ^ temp[2]) ^ mp;
            round_key[i * 4 + 3] = (round_key[(i - 4) * 4 + 3] ^ temp[3]) ^ mp;
        } else {
            round_key[i * 4 + 0] = (round_key[(i - 4) * 4 + 0] ^ m) ^ (temp[0] ^ mpt[0]);
            round_key[i * 4 + 1] = (round_key[(i - 4) * 4 + 1] ^ m) ^ (temp[1] ^ mpt[1]);
            round_key[i * 4 + 2] = (round_key[(i - 4) * 4 + 2] ^ m) ^ (temp[2] ^ mpt[2]);
            round_key[i * 4 + 3] = (round_key[(i - 4) * 4 + 3] ^ m) ^ (temp[3] ^ mpt[3]);
        }
    }

    for (int i = 40; i < 44; i++) {
        uint8_t temp[4];
        for (int j = 0; j < 4; j++) {
            temp[j] = round_key[(i - 1) * 4 + j];
        }

        if (i % 4 == 0) {
            uint8_t k = temp[0] ^ mpt[0];
            temp[0] = temp[1] ^ mpt[1];
            temp[1] = temp[2] ^ mpt[2];
            temp[2] = temp[3] ^ mpt[3];
            temp[3] = k;

            temp[0] = Sbox_masked[temp[0]];
            temp[1] = Sbox_masked[temp[1]];
            temp[2] = Sbox_masked[temp[2]];
            temp[3] = Sbox_masked[temp[3]];

            temp[0] ^= rcon[i / 4 - 1];

            round_key[i * 4 + 0] = (round_key[(i - 4) * 4 + 0] ^ temp[0]) ^ m ^ mpt[0];
            round_key[i * 4 + 1] = (round_key[(i - 4) * 4 + 1] ^ temp[1]) ^ m ^ mpt[1];
            round_key[i * 4 + 2] = (round_key[(i - 4) * 4 + 2] ^ temp[2]) ^ m ^ mpt[2];
            round_key[i * 4 + 3] = (round_key[(i - 4) * 4 + 3] ^ temp[3]) ^ m ^ mpt[3];
        } else {
            round_key[i * 4 + 0] = (round_key[(i - 4) * 4 + 0] ^ temp[0]) ^ m ^ mpt[0];
            round_key[i * 4 + 1] = (round_key[(i - 4) * 4 + 1] ^ temp[1]) ^ m ^ mpt[1];
            round_key[i * 4 + 2] = (round_key[(i - 4) * 4 + 2] ^ temp[2]) ^ m ^ mpt[2];
            round_key[i * 4 + 3] = (round_key[(i - 4) * 4 + 3] ^ temp[3]) ^ m ^ mpt[3];
        }
    }
}

/**
 * @brief Executes the AES encryption algorithm with masking techniques.
 * @param key The encryption key.
 * @param pt The plaintext array (16 bytes).
 * @param sbox The S-box array.
 * @param m The main mask.
 * @param m_prime The secondary mask.
 * @param masks The array of mask transformations.
 */
void aes_masked(const uint8_t *key,uint8_t pt[16], uint8_t m, uint8_t m_prime, uint8_t masks[4]) {

    uint8_t round_key[176];
    uint8_t ciphered_text[16];
    uint8_t x[4][4];

    init(x, pt);
    init_masked_sbox(m,m_prime);

    uint8_t masks_prime[4]; 

    for (int i = 0; i < 4; i++) {
        masks_prime[i] = masks[i];
    }
    
    mix_column(masks_prime);
    
    mask_plain_text(x,masks_prime);

    key_schedule_masked(key, round_key, m, m_prime, masks_prime);

    for (int32_t round = 0 ; round < 9; round++) {
        
        add_round_key(x, round, round_key);

        sub_byte(x);

        shift_rows(x);

        change_masks(x,m_prime,masks);
        
        {
            uint8_t col[4];
            for (int8_t i = 0; i < 4; i += 1) {
                col[0] = x[0][i];
                col[1] = x[1][i];
                col[2] = x[2][i];
                col[3] = x[3][i];
                mix_column(col);
                x[0][i] = col[0];
                x[1][i] = col[1];
                x[2][i] = col[2];
                x[3][i] = col[3];
            }
        }

    }

    add_round_key(x, 9, round_key);
    sub_byte(x);
    shift_rows(x);

    add_round_key(x, 10, round_key);


    printf("Ciphertext : ");
    for (int32_t i = 0; i < 16; i += 1) {
        ciphered_text[i] = x[i % 4][i / 4];
        printf("%.2x ", ciphered_text[i]);
    }
    printf("\n");
}


int main(int argc, char *argv[]) {

    uint8_t pt[16] = {0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34};
    uint8_t key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};

    
    int robust_alea=0;

    uint8_t m;                 
    uint8_t m_prime;           
    uint8_t masks[4];     

    if (argc > 1) {
        if (strcmp(argv[1], "1") == 0) {
            robust_alea = 1;
        } else if (strcmp(argv[1], "0") == 0) {
            robust_alea = 0;
        } else {
            fprintf(stderr, "Usage: %s [0|1]\n", argv[0]);
            return 1;
        }
    } else {
        fprintf(stderr, "Usage: %s [0|1]\n", argv[0]);
        return 1;
    }

    if (robust_alea) {

        if (RAND_bytes(&m, sizeof(m)) != 1) {
            fprintf(stderr, "Erreur lors de la génération du masque m.\n");
            return 1;
        }
        if (RAND_bytes(&m_prime, sizeof(m_prime)) != 1) {
            fprintf(stderr, "Erreur lors de la génération du masque m_prime.\n");
            return 1;
        }
        if (RAND_bytes(masks, sizeof(masks)) != 1) {
            fprintf(stderr, "Erreur lors de la génération des masques M1, M2, M3, M4.\n");
            return 1;
        }

        aes_masked(key, pt, m, m_prime,masks);

        return 0;
    }

    else {
        m = (uint8_t)(rand() % 256);        
        m_prime = (uint8_t)(rand() % 256);     
        for (int i = 0; i < 4; i++) {
            masks[i] = (uint8_t)(rand() % 256); 
        }

        aes_masked(key, pt, m, m_prime,masks);
    }

    
}


